    ...
    def __getitem__(self, indice):
        if self.mode == "train":
            ...
            for idx in frame_range:
                # [h,w,c] -> [c,h,w] BGR
                img_path = self.all_frame_addr[idx]
                cur_img = np.transpose(get_inputs(img_path), [2, 0, 1])
                cur_pose = get_inputs(img_path.replace('frames', 'poses').replace('.jpg', '.npy'))
                img_batch.append(cur_img)
                pose_batch.append(cur_pose)
            ...
            return [img_batch, pose_batch], torch.zeros(1)
            ...
            for idx in frame_range:
                img_path = self.all_frame_addr[idx]
                cur_img = np.transpose(get_inputs(img_path), [2, 0, 1])
                cur_pose = get_inputs(img_path.replace('frames', 'poses').replace('.tif', '.npy'))
                img_batch.append(cur_img)
                pose_batch.append(cur_pose)
            img_batch = np.array(img_batch)
            pose_batch = np.array(pose_batch)
            ...
            if self.return_gt:
                ...
                # gt_batch[h,w]
                return [img_batch, pose_batch], gt_batch
            else:
                return [img_batch, pose_batch], torch.zeros(1)  # to unify the interface
        else:
            raise NotImplementedError
...
class shanghaiTech_dataset(common_dataset):
    def __init__(self, dir, mode='train', context_frame_num=0, border_mode='elastic', file_format='.jpg',
                 all_bboxes=None, patch_size=32, dataset='frames'):
        '''
    ...
    def dataset_init(self):
        if self.mode == 'train':
            data_dir = os.path.join(self.dir, 'training', self.dataset) 
        elif self.mode == 'test':
            data_dir = os.path.join(self.dir, 'testing', self.dataset) 
